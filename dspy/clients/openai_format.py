...
Adapters and clients can use this as the legacy/provider boundary while keeping
internal code expressed in normalized DSPy LM types.
"""
...
def document_to_openai_blocks(document: LMDocumentPart) -> list[dict[str, Any]]:
    """Convert a semantic document part to the legacy provider-shaped document block.

    LiteLLM accepts Anthropic-style `document` content blocks for citation-enabled Anthropic models. Keeping the
    semantic block here preserves the previous `Document.format()` behavior instead of flattening citeable source
    material into plain text.
    """
    block: dict[str, Any] = {"type": "document", "citations": document.citations or {"enabled": True}}
    if document.source is not None:
        block["source"] = document.source
    elif document.data is not None:
        block["source"] = {"type": "base64", "media_type": document.media_type, "data": document.data}
    elif document.path is not None:
        block["source"] = {
            "type": "base64",
            "media_type": media_type_for_path(document.path, fallback=document.media_type),
            "data": read_path_base64(document.path),
        }
    elif document.url is not None:
        block["source"] = {"type": "url", "url": document.url}
    elif document.file_id is not None:
        block["source"] = {"type": "file", "file_id": document.file_id}
    else:
        raise ValueError("Document input requires source, data, path, url, or file_id.")
...
def citation_to_part(citation: Any) -> LMCitationPart:
    ...
    metadata = {key: value for key, value in citation.items() if key not in {"text", "title", "url"}}
    metadata["provider_data"] = dict(citation)
