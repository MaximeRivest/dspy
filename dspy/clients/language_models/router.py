...
@overload
def register_lm_backend(factory: type[LanguageModel], /, *, prefix: str) -> type[LanguageModel]: ...


def register_lm_backend(factory: LMBackendFactory | type[LanguageModel], /, *, prefix: str | None = None):
    """Register a backend used by `dspy.LM`.

    Register either a routing factory or a `LanguageModel` subclass with a
    model-prefix. A factory receives the same constructor arguments as `LM` and
    returns a `LanguageModel` when it owns the model, or `None` to let the next
    factory try.
    ...
    if not inspect.isclass(factory) or not issubclass(factory, LanguageModel):
        raise TypeError("`prefix=` registration requires a LanguageModel subclass.")
    normalized_prefix = prefix.rstrip("/") + "/"

    def route_prefixed_backend(model: str, *args: Any, **kwargs: Any) -> LanguageModel | None:
...
def _reject_legacy_lm_only_kwargs(kwargs: dict[str, Any]) -> None:
    ...
    raise TypeError(
        ...
        "Instantiate the legacy `dspy.clients.lm.LM` directly, or use a concrete `LanguageModel` backend."
    )
