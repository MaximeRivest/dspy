...
if TYPE_CHECKING:
    ...
    from dspy.core.types import LMOutput, LMRequestPatch
...
class _AdapterTypeContext:
    """Context passed to `dspy.Type.generate_patch()` during adapter rendering.

    The context is intentionally provider-neutral. Custom types can inspect the
    field, signature, adapter, and LM capabilities while contributing normalized
    DSPy LM request data instead of OpenAI/LiteLLM-shaped JSON.
    """
...
class _TypeStreamParser:
    """Private normalized stream parser for native adapter type outputs."""

    def receive(self, event: LMStreamEvent) -> Any | None:
        return None
...
class Type(pydantic.BaseModel):
    ...
    def generate_patch(self, context: _AdapterTypeContext) -> Optional["LMRequestPatch"]:
        """Return a normalized adapter-rendering patch for this value, if supported.

        Subclasses should override this to participate in the normalized adapter
        pipeline. A patch can contribute message parts, whole messages, tools,
        config, and fields to remove from the render signature without depending
        on provider-specific JSON shapes.

        Returning ``None`` keeps the legacy text/marker serializer fallback.
        """
        return None

    # TODO(DSPy 3.5): remove legacy provider-shaped rendering after built-in and
    # custom types have migrated to `generate_patch(...) -> LMRequestPatch | None`.
    ...
    # TODO(DSPy 3.5): remove the marker serializer once adapter type rendering no
    # longer depends on embedding provider-shaped JSON inside prompt text.
    ...
    @classmethod
    def stream_parser(cls, context: _AdapterTypeContext) -> _TypeStreamParser | None:
        """Return a normalized stream parser for this type, if it supports native streaming."""
        return None
    ...
    @classmethod
    def parse_lm_output(cls, context: _AdapterTypeContext, output: "LMOutput") -> Optional["Type"]:
        """Parse a normalized LM output into this type, if present.

        Subclasses should override this for native response parsing. The default
        implementation delegates to the legacy provider-shaped
        `parse_lm_response()` compatibility hook.
        """
        if output.text is not None:
            parsed = cls.parse_lm_response(output.text)
            if parsed is not None:
                return parsed
        return cls.parse_lm_response(output.to_output_dict())
...
# TODO(DSPy 3.5): remove this marker-splitting compatibility function after all
# built-in and custom adapter types use normalized rendering patches.
