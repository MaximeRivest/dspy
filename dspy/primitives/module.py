...
class Module(BaseModule, metaclass=ProgramMeta):
    ...
    def set_adapter(self, adapter):
        ...
        contained within this module. A predictor's adapter takes precedence
        over `dspy.settings.adapter`, and can itself be overridden for a
        single invocation by passing `adapter=` to the call.

        Args:
            adapter: The adapter instance to use for all predictors, or `None`
                to fall back to `dspy.settings.adapter`.
    ...
    def get_adapter(self):
        ...
        Returns:
            The adapter instance used by this module's predictors, or `None`
            if none was set.
