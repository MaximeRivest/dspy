...
class XMLStreamParser(BaseStreamParser):
        ...
        while True:
            if self._current_field is None:
                ...
                if match:
                    ...
                    # Hold back a trailing partial opening tag that may still
                    # resolve into one of the target field tags.
                    safe = self._safe_open_emit_length()
                    if safe == 0:
                        break
    ...
    def _safe_open_emit_length(self) -> int:
        idx = self._buffer.rfind("<")
        if idx >= 0 and not self._open_re.search(self._buffer[idx:]):
            return idx
        return len(self._buffer)
