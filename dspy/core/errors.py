"""The typed-error table: every runtime error dspy emits, by name.

This module is the canonical copy of the programir-contract reference
`errors.py` (SEM-3): the typed-error table, which names a program may
`Raise`, and how `ExceptHandler` types match. The engine's
`dspy.programir.engine.errors` re-exports this table, so one class
identity serves the whole system. Two channels, deliberately separate:
