...
class Predict(Module, Parameter):
    ...
    The reserved keyword `adapter=` may be passed to a single invocation to
    override the adapter for that call. Resolution follows call > instance
    (`self.adapter`) > `dspy.settings.adapter` > `ChatAdapter()` precedence.
    """
    ...
    def _forward_preprocess(self, **kwargs):
        ...
        # Get the right adapter to use, with call > instance > settings precedence.
