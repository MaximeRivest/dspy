...
### Streaming with normalized `LanguageModel`s

DSPy's new normalized LM backends stream `LMStreamEvent` objects end to end. You can use them with the same
`dspy.streamify` API. Enable the normalized router, configure a streaming `LanguageModel`, and add listeners for the
...
`LMStreamEvent` path is used for `dspy.LanguageModel` backends.
