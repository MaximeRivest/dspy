...
On `forward(**kwargs)`, `BestOfN` runs a shared candidate-search engine (`dspy.predict.candidate_search`) in its independent-sampling configuration: it deep-copies the wrapped module per attempt, swaps in a fresh LM with `rollout_id = start + i` and `temperature=1.0`, and runs the call inside a `dspy.context(trace=[])` block so the per-attempt trace is isolated. It scores `reward_fn(kwargs, pred)`, keeps the best so far, and short-circuits when a reward meets `threshold`. After the loop, the winning attempt’s trace is merged back into the parent `dspy.settings.trace`, remapped onto the wrapped module’s own predictors — so a caller inspecting the trace sees the winning path, not all `N`. The full search record (every attempt, its rollout ID, reward, and error) is attached to the returned prediction; read it with `dspy.predict.candidate_search.search_record(pred)`. `fail_count` failures are tolerated per call (defaults to `N`); exceeding the budget — or ending the search with no successful attempt at all — re-raises the last error.

**`dspy.Refine(module, N, reward_fn, threshold, fail_count=None)`**
Same shape as `BestOfN`, run on the same engine in its sequential-refinement configuration: feedback flows between attempts. After a below-threshold attempt, `Refine` builds a snapshot — the module’s source code, per-predictor signatures, per-predictor I/O from the trace, the reward function’s source — and feeds it to an internal `dspy.Predict(OfferFeedback)` inside its own isolated trace context. That call returns advice keyed by module name, which becomes explicit non-authoritative `Hint` values with provenance. Each hint is realized into the *next* attempt’s throwaway deep copy before any Adapter runs: the blamed predictor’s signature gains a defaulted `hint_` input field carrying the advice, so the retry’s calls are fully defined and adapter resolution (call, then instance, then settings, then `ChatAdapter`) is untouched — no adapter is wrapped or replaced. Advice of “N/A”, or advice targeting a name the candidate doesn’t expose, is kept on the search record as an unapplied hint rather than forced into the program; a module with no `dspy.Predict` call sites at all (a leaf holding its implementation as opaque state) skips the critic and refines by resampling alone. The `hint_` input is stripped from the winning trace before it is merged back; the raw hinted execution stays inspectable on the search record.
