...
Same shape as `BestOfN`, with feedback generation between attempts. After a failed attempt, `Refine` builds a snapshot — the module’s source code, the declared refinement units’ signatures, per-predictor I/O from the trace, the reward function’s source — and feeds it to an internal `dspy.Predict(OfferFeedback)`. That call returns advice keyed by unit name. Feedback targets *declared refinement units*: the `Parameter` leaves reported by `module.named_refinement_units()` whose `accepts_hint()` returns `True` (`dspy.Predict` opts in by default; a leaf whose predictors are derived runtime state can decline, in which case `Refine` degrades to resample-and-select). On the next attempt, the advice is installed as an explicit non-authoritative `dspy.Hint` — with provenance — on the hinted units of the per-attempt deep copy. Each hinted `Predict` interprets the hint into its own effective call (an appended `hint_` input field) before its normally-resolved adapter formats it, so adapters are never wrapped or mutated, the hint exists only for that attempt, and it never leaks into the final trace. The trace shows only the winning attempt; per-attempt rollout ids, rewards, and applied hints are attached to the returned prediction via `prediction.get_refinement()`.
