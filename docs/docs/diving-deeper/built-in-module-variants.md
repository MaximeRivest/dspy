...
On `forward(**kwargs)`, `BestOfN` deep-copies the wrapped module per attempt, swaps in a fresh LM with `rollout_id = start + i` and `temperature=1.0`, and runs the call inside a `dspy.context(trace=[])` block so the per-attempt trace is isolated. It scores `reward_fn(kwargs, pred)`, keeps the best so far, and short-circuits when a reward meets `threshold`. After the loop, the winning attempt’s trace is merged back into the parent `dspy.settings.trace` — with the traced predictors mapped back to the wrapped module’s own — so a caller inspecting the trace sees the winning path through the program it owns, not all `N`. Each call tolerates up to `fail_count` failed attempts (defaults to `N`); one more failure re-raises the error, and if every attempt fails the last error is raised rather than returning `None`. `acall` mirrors the same contract.

**`dspy.Refine(module, N, reward_fn, threshold, fail_count=None)`**
Same shape as `BestOfN`, with feedback generation between attempts. After a failed attempt, `Refine` builds a snapshot — the module’s source code, per-predictor signatures, per-predictor I/O from the trace, the reward function’s source — and feeds it to an internal `dspy.Predict(OfferFeedback)`. That call returns advice keyed by module name. On the next attempt, `Refine` appends a `hint_` input field — whose default value is that predictor’s slice of the advice — to each sub-predictor’s signature on the per-attempt deep copy. The advice is a non-authoritative search hint applied above the adapter boundary: the retry is fully defined before any adapter formats a call, so adapters are never wrapped or subclassed and each predictor’s own adapter resolution (instance adapter, then settings, then `ChatAdapter`) is untouched. The hint exists only on that attempt’s throwaway copy; the exported winning trace strips it and maps traced predictors back to the wrapped module’s own. Only `dspy.Predict` leaves found via `named_predictors()` receive hints — other kinds of sub-modules are resampled and selected but not hinted.
