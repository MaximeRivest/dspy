...
On `forward(**kwargs)`, `BestOfN` deep-copies the wrapped module per attempt, swaps in a fresh LM with `rollout_id = start + i` and `temperature=1.0`, and runs the call inside a `dspy.context(trace=[])` block so the per-attempt trace is isolated. It scores `reward_fn(kwargs, pred)`, keeps the best so far, and short-circuits when a reward meets `threshold`. After the loop, the winning attempt’s trace is merged back into the parent `dspy.settings.trace`, with entries remapped to the original module’s predictors — so a caller inspecting the trace sees the winning path through the program it owns, not all `N`. Failures are counted per call against `fail_count` (defaults to `N`; `0` tolerates none); exceeding the budget re-raises the most recent error, and if every attempt fails the last error is raised rather than returning `None`.

**`dspy.Refine(module, N, reward_fn, threshold, fail_count=None)`**
Same shape as `BestOfN`, with feedback generation between attempts. After a scored attempt that misses the threshold, `Refine` builds a snapshot — the module’s source code, per-predictor signatures, per-predictor I/O from the trace, the reward function’s source — and feeds it to an internal `dspy.Predict(OfferFeedback)`. That call returns advice keyed by predictor name. On the next attempt, each predictor on that attempt’s throwaway deep copy gains a `hint_` input field whose default value is its slice of the advice, so the retry is fully defined before any adapter formats a call — adapters are never wrapped or subclassed, and the wrapped module is never mutated. The hint is stripped from the trace merged back to the caller. A critic failure is logged and does not consume the failure budget; a predictor that already declares `hint_` keeps it and receives no critic hint.
