...
**`dspy.Refine(module, N, reward_fn, threshold, fail_count=None, feedback_policy="auto")`**
Same shape as `BestOfN`, with feedback between attempts. The reward function may speak for itself: returning `dspy.Prediction(score=..., feedback="...")` (the GEPA metric shape) instead of a bare float makes the feedback the retry's hint directly. When the eval returns only a score, `feedback_policy` decides what happens next: `"auto"` (default) falls back to an internal `dspy.Predict(OfferFeedback)` critic that reads the module's source, per-predictor I/O from the trace, and the reward function's source, and returns advice keyed by module name; `"eval"` never invokes the critic; `"none"` disables hints entirely. Hints are non-authoritative and applied above the adapter: the next attempt's per-attempt deep copy gets an explicit `hint_` input field appended to each targeted predictor's signature (its description names the hint's source and non-authoritative status), so the hint flows through whatever adapter the predictor resolves — call, then instance, then settings, then `ChatAdapter` — without wrapping or mutating any adapter. Only the winning attempt's trace is merged back, with `hint_` stripped so the trace matches the wrapped program's declared signatures; the full per-attempt record (scores, feedback, hints with provenance) rides on the returned prediction as `_refinement`.
