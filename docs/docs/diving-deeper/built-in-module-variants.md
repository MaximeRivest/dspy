...
**`dspy.Refine(module, N, reward_fn, threshold, fail_count=None, *, feedback_fn=None, hint_field="hint")`**
Same shape as `BestOfN`, plus an *explicit* feedback loop. Without `feedback_fn`, `Refine` is `BestOfN`: a scalar reward doesn’t explain why an attempt failed, so `Refine` no longer manufactures an explanation automatically. To guide retries, you declare both halves: predictors whose Signature declares an input field named `hint_field` are the only ones that can receive advice, and `feedback_fn(inputs, prediction, reward)` — optionally declaring keyword parameters `trace`, `module`, `threshold`, or `hint_field` to receive them — is the only source of advice. `dspy.LMCritic(objective=...)` is an opt-in LM critic that reasons from your declared objective, the modules’ declared signatures, and the failed run’s evidence; it performs no source-code archaeology. Hints (`dspy.Hint`) are non-authoritative, scoped to the next attempt, and delivered by filling the declared field’s default on the per-attempt deep copy — the adapter is never wrapped or subclassed, and a value the module’s own code passes for the field wins over any hint. Feedback runs in an isolated trace; a hint with no declared destination is recorded as ignored, never force-fitted. The returned prediction carries a `RefinementReport` (per-attempt rewards, hints with provenance, errors) on its `_refinement` attribute.
