...
The first message in that list is the system message, so slicing `adapter.format(...)[0]` shows you exactly how the adapter presents your signature's fields and instructions to the model. (Adapters also expose finer-grained `format_*` helper methods you may see in older code; they still work, but they are the legacy customization surface — prefer inspecting via `format()` and, for real prompt control, watch for the template system described in the [adapters deep-dive](../../diving-deeper/adapters.md).)
