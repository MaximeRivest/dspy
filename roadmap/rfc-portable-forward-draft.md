...
**The inputs record (v0.4):** a forward may take its declared inputs as one record parameter (`def forward(self, inputs)`) typed by the module's own signature, and pass it through to a call (`self.predict(**inputs)`). This looks dynamic but isn't: the record's key set is pinned in the artifact by the signature, so validators and other-language runtimes see a fully static call. It exists because *signature-polymorphic* modules — ReAct is the canonical case — are otherwise unwritable as normal functions. The line it does not cross: a dict whose keys the *model* chooses at runtime still refuses. Splat what the signature declares, never what the model decides.
