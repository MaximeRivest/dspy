...
| 2 | **signature** | per predictor: ordered `fields[]` each `{name, direction(input/output), desc, shape, semantic_role}` (`prefix` deprecated out, D-051 — the live field argument is deprecated and rendering prefixes derive from field names in component-4 templates; readers still accept the key on old bytes) — `direction` is the renamed old `role` key; **`shape`** is the field's data type as JSON schema (*arbitrary* Python/pydantic typing — no blessed type list; hints with no JSON-schema meaning, e.g. `Callable`, refuse loudly: those are tools, not data) **including the field's declared constraints** (`ge`/`le`/`min_length`/… land as `minimum`/`maximum`/`minLength`/… — D-051: no separate constraint slot, and dropping them is an exporter bug); **`semantic_role`** is the field's relationship to the exchange, a closed versioned vocabulary (`plain \| reasoning \| tools \| tool_calls \| citations \| history \| media \| code`) — intent, hence frozen: "I want cited answers" defines the task; *how* citations are obtained is a component-4 strategy (§Adapter-semantics). **Instructions are NOT stored here** — they live once in 3a (see note) | JSON (mirrors `Signature.dump_state()`, `signature.py:495`, minus instructions) | frozen | bake |
...
> Diagrams are inline mermaid — the sources below are authoritative and render
> natively on GitHub. (The old `../reports/diagrams/*.png` renders are retired.)
...
flowchart TB
  subgraph PROG["ProgramIR — one self-contained, trainable unit"]
    ...
    subgraph BAKE["BAKED · travels in the artifact"]
      ...
      FWD["5 · forward AST<br/>restricted Python, whitelist,<br/>calls → typed leaves"]
      TOOL["6 · tools<br/>identity + Python source + deps"]
      INT["7 · interpreter<br/>identity profile + placement"]
    ...
    subgraph DECL["DECLARED · verified at load, refused loudly"]
      direction TB
      LMID["8a · LM contract identity<br/>typed_lm — canonical: lm15<br/>Request/Response/StreamEvent"]
  ...
  class MT,SIG,LEARN,ADP,FWD,TOOL,INT,W,ENV,AMB,MET bake;
  class LMID,SYS decl;
  class SEC cred;
```
...
flowchart LR
  subgraph OPT["optimizable surface (a subset of BAKED)"]
    T["3a instructions · 4 literal-table<br/>values · strategy literals<br/><i>text</i>"]
    D["3b demos<br/><i>demos</i>"]
    C["3c config · 4 output_structure ·<br/>bindings (adapter/strategy/codec) ·<br/>lowering application<br/><i>choice</i>"]
    B["6 tool bodies · strategy parsers<br/>(under seed regimes, §e2)<br/><i>authored-code</i>"]
    WR["8b weights + deltas<br/><i>weight-ref</i>"]
    ST["1/5 tree + forward rewrites<br/>within the §d grammar<br/><i>structure (§e2)</i>"]
  end
  OPT -->|"⊆"| BAKED["everything baked<br/>(the full artifact)"]
  FROZEN["frozen: external signature (type),<br/>metric + devset (value, comp. 12),<br/>plan/transform/parser machinery,<br/>interpreter profile, invariants"] --> BAKED
  classDef o fill:#0d47a1,stroke:#90caf9,color:#fff;
  classDef b fill:#1b5e20,stroke:#a5d6a7,color:#fff;
  class T,D,C,B,WR,ST o; class BAKED,FROZEN b;
```
...
flowchart TB
  ...
  ROOT --> M["manifest.json<br/><small>components 1,2,3,4,5,6-identity,7,8a,9-sys,10,11,12-identity</small>"]
  ROOT --> W["weights/"]
  ROOT --> T["tools/"]
...
sequenceDiagram
  ...
  Bob->>Bob: optionally resume training — every optimizable field is baked
```
...
- **Role is intent → component 2**, the frozen `semantic_role` slot. The
  ...
  **Empirical confirmation, and the canonical contract (ratified
  ...
  flowchart TB
    ...
    class P1,P2,P3,P4,P5,P6 c; class R1,R2,R3,R4,R5 r; class STRAT s;
  ```
  ...
  *undeclared* reasoning stream into the observability channel
  ...
  ```mermaid
  flowchart LR
    LM["LM stream<br/>start · delta* · end | error"] --> RT{"route by<br/>delta type × plan"}
    RT -->|"ThinkingDelta,<br/>role DECLARED"| DF["field stream:<br/>partial reasoning value"]
    RT -->|"ThinkingDelta,<br/>role UNDECLARED"| TJ["_trajectory<br/>(exhaust, live)"]
    RT -->|"ToolCallDelta /<br/>CitationDelta"| SP["role's strategy parser<br/>owns the fold"]
    RT -->|"TextDelta<br/>(textual strategies)"| FP["format section-parser splits<br/>(StreamListener = the polyfill)"]
    DF & SP & FP --> PRED["Prediction<br/>(= declared fields, exactly)"]
    classDef s fill:#0d47a1,stroke:#90caf9,color:#fff;
    classDef d fill:#1b5e20,stroke:#a5d6a7,color:#fff;
    classDef x fill:#37474f,stroke:#b0bec5,color:#fff;
    class LM,RT s; class DF,SP,FP,PRED d; class TJ x;
  ```
...
```mermaid
flowchart LR
  SIG["signature<br/>(intent: shapes + roles)"] -->|"program makers<br/>originate structure"| SURF["SURFACE TREE<br/>module tree +<br/>sugar annotations"]
  SURF -->|"lowerings<br/>rewrite tree"| CORE["CORE TREE<br/>only typed leaves,<br/>bindings, tags —<br/>the ONLY serialized form"]
  CORE -->|"formats + strategies<br/>+ codecs render"| WIRE["wire<br/>LMRequest → LMResponse"]
  classDef intent fill:#4a148c,stroke:#ce93d8,color:#fff;
  classDef tree fill:#1b5e20,stroke:#a5d6a7,color:#fff;
  classDef wire fill:#0d47a1,stroke:#90caf9,color:#fff;
  class SIG intent; class SURF,CORE tree; class WIRE wire;
```

The four mechanism layers, each scoped one level down and stratified by the
single-shot law (a strategy may never add a call; a format may never touch
structure; a strategy may never own format literals):
...
flowchart TB
  ...
  classDef intent fill:#4a148c,stroke:#ce93d8,color:#fff;
  class L,F,S,CD mech; class INTENT intent;
```
...
```mermaid
flowchart LR
  subgraph LADDER["the placement ladder — same contract at every rung"]
    direction LR
    R0["rung 0 · BAKE<br/>in-process<br/>(bytes/source travel)"] --> R1["local RPC /<br/>same-sandbox<br/>subprocess"] --> R2["HTTP local /<br/>separate sandbox"] --> R3["HTTP remote /<br/>MCP / provider<br/>· DECLARE"]
  end
  C["invariant typed contract<br/>LM: forward(Request)→Response<br/>interpreter: execute(code,vars)→result<br/>tool: LMToolSpec + return_schema"] -.->|"identical at every rung"| LADDER
  classDef bake fill:#1b5e20,stroke:#a5d6a7,color:#fff;
  classDef decl fill:#e65100,stroke:#ffcc80,color:#fff;
  classDef mid fill:#37474f,stroke:#b0bec5,color:#fff;
  class R0 bake; class R1,R2 mid; class R3 decl; class C mid;
```
...
```mermaid
flowchart LR
  S0["IR₀<br/>baseline<br/>0.42"] -->|"diff: 3a instructions<br/>(MIPRO)"| S1["IR₁<br/>0.55"]
  S1 -->|"diff: 4 strategy binding<br/>citations: native→span_markers"| S2["IR₂<br/>0.61"]
  S2 -->|"diff: structure<br/>lowering: two_step applied"| S3["IR₃<br/>0.66"]
  S1 -->|"diff: 3b demos<br/>(rejected: 0.51)"| X1["IR₁′<br/>abandoned"]
  S3 -->|"ship → receiver deviation:<br/>swap LM entry, re-scored"| S4["IR₄ (fork)<br/>0.63 on their metric"]
  classDef kept fill:#1b5e20,stroke:#a5d6a7,color:#fff;
  classDef dead fill:#37474f,stroke:#b0bec5,color:#fff;
  classDef fork fill:#0d47a1,stroke:#90caf9,color:#fff;
  class S0,S1,S2,S3 kept; class X1 dead; class S4 fork;
```

Every node is a complete, reconstructable `ProgramIR` (checkpoint = save);
content-addressing makes the whole tree cost kilobytes over the shared frozen
blobs; a receiver's recorded deviation continues the same trajectory across
owners. Because the diff is between two IRs and the changed fields are exactly
the `optimizable ⊆ baked` set,
every step is a **precise, replayable, machine-readable rewrite** —
