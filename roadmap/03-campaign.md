...
- ~~We will need to redo flex~~ — **underway (2026-08-11)**: FlexIR rewrites the program IR itself via a reflection LM with a closed edit vocabulary — including replacing predict leaves with generated ordinary-Python tool leaves (cheapness as an objective, metric-gated, holdout against reward hacking). The string-substrate flex and its "string-only new leaf" hack die with it. v1 (data edits) landed on greenfield; v2 (leaf rewrites) follows the flex-study brief.
