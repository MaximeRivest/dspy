...
class AdapterEchoLM(dspy.LanguageModel):
            ...
            super().__init__(
                response=dspy.LMResponse(
                    ...
                    outputs=[
                        dspy.LMOutput(
                            parts=[
                                dspy.LMTextPart(text="I should call search."),
                                dspy.LMToolCallPart(name="search", args={"query": "DSPy"}),
                            ]
                        )
                    ],
                )
...
def test_adapter_extracts_native_reasoning_from_normalized_language_model_outputs():
    class ReasoningLM(dspy.LanguageModel):
        def __init__(self):
        ...
        def forward(self, request: dspy.LMRequest) -> dspy.LMResponse:
            assert request.config.reasoning == dspy.LMReasoningConfig(effort="low")
            return dspy.LMResponse(
                model=request.model,
                outputs=[
                    dspy.LMOutput(
                        parts=[
                            dspy.LMThinkingPart(text="Think first."),
...
def test_json_adapter_uses_language_model_capabilities_not_legacy_properties():
    ...
    assert not hasattr(lm, "supported_params")
