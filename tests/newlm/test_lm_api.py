    ...
    def forward(self, request: dspy.LMRequest) -> dspy.LMResponse:
        ...
        return dspy.LMResponse(
            ...
            outputs=[
                dspy.LMOutput(
                    parts=[
                        dspy.LMThinkingPart(text="The calls are independent, so I can request them in parallel."),
                        dspy.LMToolCallPart(
                            id="call_sf",
...
def test_simple_call_normalizes_to_lm_request():
    ...
    assert request.messages[0].parts == [dspy.LMTextPart(text="hello")]
...
def test_openai_messages_are_boundary_input_not_internal_contract():
    ...
    assert request.messages[0].parts == [dspy.LMTextPart(text="Be terse.")]
    assert request.messages[1].parts == [dspy.LMTextPart(text="What is DSPy?")]
...
def test_message_constructors_create_multiturn_lm_messages():
    ...
    assert request.messages[-1].parts == [dspy.LMTextPart(text="Say that in five words.")]
...
def test_multimodal_positional_parts_normalize_into_one_user_message():
    ...
    assert request.messages[0].parts == [
        dspy.LMTextPart(text="describe this image"),
        dspy.LMImagePart(data="abc123", media_type="image/png"),
    ]
...
def test_python_tools_normalize_to_lm_tool_specs():
    ...
    assert request.tools[0] == dspy.LMToolSpec(
        name="get_current_weather",
...
def test_tool_results_can_be_sent_back_as_normalized_tool_messages():
    ...
    lm(
        ...
        dspy.Assistant(
            dspy.ToolCall(id="call_paris", name="get_current_weather", args={"location": "Paris, France"})
        ),
        dspy.ToolResult(
            call_id="call_paris",
            name="get_current_weather",
    ...
    assert request.messages[2].parts == [
        dspy.LMToolResultPart(
            call_id="call_paris",
            name="get_current_weather",
            content=[dspy.LMTextPart(text='{"location": "Paris", "temperature": "22", "unit": "celsius"}')],
        )
    ]
