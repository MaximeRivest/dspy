...
def test_normalize_reasoning_config_from_legacy_effort_kwarg():
    ...
    assert request.config.reasoning == dspy.LMReasoningConfig(effort="high")


def test_normalize_reasoning_config_from_explicit_object():
    reasoning = dspy.LMReasoningConfig(effort="high", summary="auto")
...
def test_normalize_cache_config():
    ...
    assert request.config.cache == dspy.LMCacheConfig(enabled=False, rollout_id=123)
...
def test_normalize_image_audio_file_and_reasoning_parts_in_one_user_message():
    ...
    assert request.messages == [
        dspy.User(
            "summarize these inputs",
            dspy.LMImagePart(data="image-bytes", media_type="image/png"),
            dspy.LMAudioPart(data="audio-bytes", media_type="audio/wav"),
            dspy.LMFilePart(data="file-bytes", media_type="application/pdf", filename="paper.pdf"),
            dspy.LMThinkingPart(text="Prior reasoning supplied by the caller."),
        )
    ]
...
def test_normalize_previous_lm_response_as_assistant_message():
    previous = dspy.LMResponse(
        ...
        outputs=[
            dspy.LMOutput(
                parts=[
                    dspy.LMThinkingPart(text="I should answer concisely."),
                    dspy.LMTextPart(text="DSPy is a programming framework."),
                    dspy.LMCitationPart(text="DSPy documentation", title="DSPy Docs", url="https://dspy.ai"),
                    dspy.LMImagePart(data="image-bytes", media_type="image/png"),
                ]
            )
        ],
...
def test_normalize_tool_call_and_tool_result_messages():
    request = make_lm().normalize_request(
        ...
        dspy.ToolResult(
            ...
            content=[
                ...
                dspy.LMTextPart(text='{"temperature": "22", "unit": "celsius"}'),
    ...
    assert request.messages == [
        dspy.User("What is the weather in Paris?"),
        dspy.Assistant(dspy.LMToolCallPart(id="call_1", name="get_weather", args={"location": "Paris"})),
        dspy.ToolResult(
            dspy.LMToolResultPart(
                call_id="call_1",
                name="get_weather",
                content=[
                    dspy.LMTextPart(text="Here is the tool result: "),
                    dspy.LMTextPart(text='{"temperature": "22", "unit": "celsius"}'),
                    dspy.LMImagePart(data="weather-chart", media_type="image/png"),
                ],
            )
        ),
        dspy.User("Summarize."),
...
def test_normalize_bare_lm_parts_into_user_message():
    request = make_lm().normalize_request(
        dspy.LMTextPart(text="describe this generated image"),
        dspy.LMImagePart(data="generated-image", media_type="image/png"),
        dspy.LMCitationPart(text="source text", title="Source", url="https://example.com/source"),
    )
...
def test_normalize_positional_tool_sugar():
    ...
    assert request.messages == [
        dspy.User("crop the dog", dspy.LMImagePart(data="image-bytes", media_type="image/png"))
    ]
...
def test_normalize_explicit_tools_and_tool_choice():
    ...
    assert request.config.tool_choice == dspy.LMToolChoice(mode="required", parallel=False)
