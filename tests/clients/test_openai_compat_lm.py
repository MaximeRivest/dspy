...
@pytest.mark.parametrize(
    ...
    [
        (400, _envelope("context_length_exceeded", "too long"), ContextWindowExceededError, "context_length_exceeded"),
...
def test_normalize_error_maps_exact_error_class(status, body, expected_cls, expected_code):
...
def test_normalize_error_uses_type_when_custom_code_is_unknown():
...
def test_normalize_error_coerces_non_string_messages(message):
...
def test_normalize_error_extracts_response_headers():
...
def test_forward_translates_request_and_response():
    post = FakePost(_raw(_completion("answer")))
...
def test_extra_headers_explicitly_override_defaults():
    post = FakePost()
...
def test_transport_exceptions_are_normalized(exception, expected_cls):
    lm = _make_lm(post=FakePost(exception))
...
def test_http_errors_are_raised_from_forward():
    lm = _make_lm(post=FakePost(_raw({"error": {"message": "slow"}}, status=429)))
...
def test_invalid_success_responses_raise_provider_error(body):
    lm = _make_lm(post=FakePost(_raw(body)))

    with pytest.raises(LMProviderError):
        lm.forward(dspy.LMRequest.from_call(model=MODEL, prompt="hi"))


def test_retryable_errors_retry_and_non_retryable_errors_do_not(monkeypatch):
    sleeps = []
    monkeypatch.setattr("dspy.clients.openai_compat_lm.time.sleep", sleeps.append)
    retrying_post = FakePost(
        _raw({"error": {"message": "slow"}}, status=429, headers={"retry-after": "0.25"}),
        _raw(_completion("ok")),
    )
    lm = _make_lm(post=retrying_post, num_retries=2)

    assert lm.forward(dspy.LMRequest.from_call(model=MODEL, prompt="hi")).text == "ok"
    assert len(retrying_post.calls) == 2
    assert sleeps == [0.25]

    invalid_post = FakePost(_raw({"error": {"message": "bad"}}, status=400), _raw(_completion()))
    lm = _make_lm(post=invalid_post, num_retries=2)
    with pytest.raises(LMInvalidRequestError):
        lm.forward(dspy.LMRequest.from_call(model=MODEL, prompt="hi"))
    assert len(invalid_post.calls) == 1
...
def test_cache_avoids_second_post_and_cache_false_overrides(isolated_cache):
    post = FakePost(_raw(_completion("cached")), _raw(_completion("fresh")), _raw(_completion("again")))
...
def test_cache_identity_distinguishes_endpoint_and_credentials(isolated_cache):
    request = dspy.LMRequest.from_call(model=MODEL, prompt="same")
    first_post = FakePost(_raw(_completion("first")))
    second_post = FakePost(_raw(_completion("second")))
    third_post = FakePost(_raw(_completion("third")))
...
@pytest.mark.asyncio
async def test_aforward_matches_forward():
    sync_lm = _make_lm(post=FakePost(_raw(_completion("same"))))
    async_lm = _make_lm(post=FakePost(_raw(_completion("same"))))
...
def test_legacy_and_typed_public_returns():
    lm = _make_lm(post=FakePost(_raw(_completion("legacy")), _raw(_completion("typed"))))
...
def test_api_key_resolution(monkeypatch, kwargs, env, expected):
...
def test_capabilities_default_false_and_honor_opt_in(flag):
    assert getattr(_make_lm(), flag) is False
    assert getattr(_make_lm(**{flag: True}), flag) is True


def test_supported_params_follow_capabilities():
    lm = _make_lm(supports_function_calling=True, supports_reasoning=True, supports_response_schema=True)

    assert {"tools", "tool_choice", "parallel_tool_calls", "reasoning_effort", "response_format"} <= lm.supported_params
...
def test_invalid_configuration_raises_structured_error(kwargs, match):
    with pytest.raises(dspy.LMConfigurationError, match=match):
        _make_lm(**kwargs)


def test_dump_load_round_trip_is_safe_and_uses_generic_loader(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("GATEWAY_KEY", raising=False)
    lm = _make_lm(
...
def test_openai_compat_lm_is_exported_at_top_level():
