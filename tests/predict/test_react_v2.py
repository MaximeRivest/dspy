...
def test_react_v2_text_mock_lm_loop_records_inputs_once():
    ...
    assert pred._trajectory["termination_reason"] == "submit"
    assert sum("question" in event for event in pred._trajectory["history"].messages) == 1
    assert pred._trajectory["history"].messages[0]["tool_calls"].tool_calls[0].id == "call_0_0"
    assert "tool_call_results" not in pred._trajectory["history"].messages[0]
    assert pred._trajectory["history"].messages[0]["tool_calls"].tool_call_results.tool_call_results[0].call_id == "call_0_0"
...
def test_react_v2_text_mode_accepts_top_level_tool_arguments():
    ...
    assert pred.answer == "found cats"
    assert pred._trajectory["termination_reason"] == "submit"
    assert pred._trajectory["history"].messages[0]["tool_calls"].tool_calls[0].args == {"query": "cats"}
...
def test_react_v2_unknown_tool_observation_can_continue():
    ...
    first_result = pred._trajectory["history"].messages[0]["tool_calls"].tool_call_results.tool_call_results[0]
...
def test_react_v2_accepts_serialized_history_input():
    ...
    assert pred.answer == "done"
    assert pred._trajectory["history"].messages[0] == {"question": "old"}
    assert all(event for event in pred._trajectory["history"].messages)
...
def test_react_v2_forced_submit_on_empty_tool_calls():
    ...
    assert pred._trajectory["termination_reason"] == "forced_submit"
...
def test_react_v2_native_tool_loop_replays_tool_result_with_provider_id():
    ...
    assert pred.answer == "found cats"
    assert pred._trajectory["history"].messages[0]["tool_calls"].tool_calls[0].id == "call_provider_1"
    assert "tool_call_results" not in pred._trajectory["history"].messages[0]
    assert pred._trajectory["history"].messages[0]["tool_calls"].tool_call_results.tool_call_results[0].call_id == "call_provider_1"
    assert any(
...
def test_react_v2_native_parallel_tool_calls_are_requested_and_replayed():
    ...
    assert [call.id for call in pred._trajectory["history"].messages[0]["tool_calls"].tool_calls] == [
        "call_provider_1",
        "call_provider_2",
    ]
    assert [
        result.call_id
        for result in pred._trajectory["history"].messages[0]["tool_calls"].tool_call_results.tool_call_results
    ] == [
        "call_provider_1",
        "call_provider_2",
    ]
